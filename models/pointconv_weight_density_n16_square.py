...
import numpy as np
import tf_util
from PointConv_square import pointnet_fp_module, feature_encoding_layer, feature_decoding_layer
...
#    l0_points = feature_decoding_layer(l0_xyz, l1_xyz, l0_points, l1_points, 0.1, sigma, 16, [128,128,128], is_training, bn_decay, weight_decay, scope='fa_layer4')
    # Feature encoding layers
    l1_xyz, l1_points = feature_encoding_layer(l0_xyz, l0_points, npoint=2048,size=3,layers=1,stride=2, mlp=32, radius=0.05, sigma=sigma, is_training=is_training, bn_decay=bn_decay, weight_decay = weight_decay, scope='layer1')
    l1_xyz, l1b_points = feature_encoding_layer(l1_xyz, l1_points, npoint=2048,size=3,layers=1,stride=1,mlp=32, radius=0.05, sigma=sigma, is_training=is_training, bn_decay=bn_decay, weight_decay = weight_decay, scope='layer1b')
    l2_xyz, l2_points = feature_encoding_layer(l1_xyz, l1b_points, npoint=1024,size=3,layers=1,stride=2, mlp=64, radius=0.1, sigma=2*sigma, is_training=is_training, bn_decay=bn_decay, weight_decay = weight_decay, scope='layer2')
    l2_xyz, l2b_points = feature_encoding_layer(l2_xyz, l2_points, npoint=1024,size=3,layers=1,stride=1, mlp=64, radius=0.1, sigma=2*sigma, is_training=is_training, bn_decay=bn_decay, weight_decay = weight_decay, scope='layer2b')
    ...
    l3_xyz, l3_points = feature_encoding_layer(l2_xyz, l2b_points, npoint=512, size=3,layers=1,stride=2,mlp=128, radius=0.2, sigma=4*sigma, is_training=is_training, bn_decay=bn_decay, weight_decay = weight_decay, scope='layer3')
    l3_xyz, l3b_points = feature_encoding_layer(l3_xyz, l3_points, npoint=512, size=3,layers=1,stride=1,mlp=128, radius=0.2, sigma=4*sigma, is_training=is_training, bn_decay=bn_decay, weight_decay = weight_decay, scope='layer3b')
    l4_xyz, l4_points = feature_encoding_layer(l3_xyz, l3b_points, npoint=256,size=3,layers=1,stride=2, mlp=256, radius=0.4, sigma=8*sigma, is_training=is_training, bn_decay=bn_decay, weight_decay = weight_decay, scope='layer4')
    l4_xyz, l4b_points = feature_encoding_layer(l4_xyz, l4_points, npoint=256,size=3,layers=1,stride=1,mlp=256, radius=0.4, sigma=8*sigma, is_training=is_training, bn_decay=bn_decay, weight_decay = weight_decay, scope='layer4b')
    l5_xyz, l5_points = feature_encoding_layer(l4_xyz, l4b_points, npoint=128,size=3,layers=1,stride=2, mlp=512, radius=0.8, sigma=16*sigma, is_training=is_training, bn_decay=bn_decay, weight_decay = weight_decay, scope='layer5')
    l5_xyz, l5b_points = feature_encoding_layer(l5_xyz, l5_points, npoint=128,size=3,layers=1,stride=1, mlp=512, radius=0.8, sigma=16*sigma, is_training=is_training, bn_decay=bn_decay, weight_decay = weight_decay, scope='layer5b')
