...
def kernel_density_estimation_ball(pts, feature,radius, sigma, N_points = 128, is_norm = False):
    with tf.variable_scope("ComputeDensity") as sc:
        idx, pts_cnt = tf_grouping.query_ball_point(radius, N_points, pts, pts)
        g_pts = tf_grouping.group_point(feature, idx)
        g_pts -= tf.tile(tf.expand_dims(feature, 2), [1, 1, N_points, 1])
