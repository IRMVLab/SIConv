...
parser.add_argument('--model', default='scannet_pickle', help='Model name [default: model]')
parser.add_argument('--data_root', default='scannet_pickle', help='Dataset dir [default: scannet_pickle]')
parser.add_argument('--log_dir', default='log', help='Log dir [default: log]')
parser.add_argument('--num_point', type=int, default=8192, help='Point Number [default: 8192]')
parser.add_argument('--max_epoch', type=int, default=3000, help='Epoch to run [default: 3000]')
...
parser.add_argument('--restore_epoch', type=int, default=0, help='Restore epoch e.g. 200 [default: None]')
...
if not os.path.exists('train_result'): os.mkdir('train_result')
...
DATA_PATH = FLAGS.data_root#os.path.join(BASE_DIR, 'scannet')
print("start loading training data ...")
TRAIN_DATASET = scannet_dataset_rgb.ScannetDataset(root=DATA_PATH, block_points=NUM_POINT, split='train',with_rgb=False)
print("start loading validation data ...")
TEST_DATASET = scannet_dataset_rgb.ScannetDataset(root=DATA_PATH, block_points=NUM_POINT, split='val',with_rgb=False)
print("start loading whole scene validation data ...")
TEST_DATASET_WHOLE_SCENE = scannet_dataset_rgb.ScannetDatasetWholeScene(root=DATA_PATH, block_points=NUM_POINT, split='val',with_rgb=False)
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        loss_sum += loss_val
        total_IoU=total_correct/float(total_iou_deno)
        if (batch_idx+1)%10 == 0:
            log_string(' -- %03d / %03d --' % (batch_idx+1, num_batches))
            log_string('mean loss: %f' % (loss_sum / 10))
            log_string('accuracy: %f' % (total_correct / float(total_seen)))
            log_string('total IoU: %f' % (total_correct / float(total_iou_deno)))
            tf.summary.scalar('total IoU', total_IoU)
            total_correct = 0
            total_seen = 0
            loss_sum = 0
            total_iou_deno = 0
