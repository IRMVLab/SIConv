...
sys.path.append(os.path.join(BASE_DIR, 'tf_ops/ops_sphere'))
...
def feature_encoding_layer(xyz, feature, npoint,size,layers,stride, mlp, radius, sigma, is_training, bn_decay, weight_decay, scope, relative_xyz=False, bn=True, use_xyz=True):
    ''' Input:
            xyz: (batch_size, ndataset, 3) TF tensor
            feature: (batch_size, ndataset, channel) TF tensor
            npoint: int32 -- #points sampled in farthest point sampling
    ...
    with tf.variable_scope(scope) as sc:
        ...
        feature_density = nonlinear_transform(grouped_density, [32, 16, 1], scope='density_net', is_training=is_training,
        ...
        #feature_density(b,n,1)
        N_points = 64
        idx, pts_cnt = tf_grouping.query_ball_point(l, N_points, xyz, xyz)
        xyz_pts = tf_grouping.group_point(xyz, idx)#(b,n,nsample,3)
        feature_pts = tf_grouping.group_point(feature, idx)#(b,n,nsample,c)

        grouped_density = tf.concat([xyz_pts, feature_pts], axis=3)
        feature_density=tf_util.conv2d(grouped_density, 32, kernel_size=[1,1],
                                      padding='VALID',data_format='NHWC', stride=[1,1],
                                      bn=bn, is_training=is_training,
                                      scope='density_net1', bn_decay=bn_decay, weight_decay = weight_decay)#(b,n,128,32)

        #feature_density = tf_util.conv1d(grouped_density, 32, 1,
        #                                padding = 'VALID', stride=1,
        #                                bn = True, is_training = is_training, activation_fn=tf.nn.relu,
        #                                scope = 'density_net1', bn_decay=bn_decay, weight_decay = weight_decay)#(b,n,128,32)
        feature_density = tf.reduce_max(feature_density, axis=2)#(b,n,32)
        feature_density = nonlinear_transform(feature_density, [16, 1], scope='density_net2', is_training=is_training,
                                              bn_decay=bn_decay, weight_decay=weight_decay)


        """
        grouped_xyz, grouped_feature, _ = pointconv_util.grouping(feature, K, xyz, xyz, use_xyz=False)
        xyz_k = tf.tile(tf.expand_dims(xyz, axis=2), (1, 1, K, 1))  # (b,n,K,3)
        feature_k = tf.tile(tf.expand_dims(feature, axis=2), (1, 1, K, 1))  # (b,n,K,c)
        group_xyz_l1 = xyz_k - grouped_xyz  # (b,n,K,3)
        grouped_feature_l1 = feature_k - grouped_feature  # (b,n,K,c)
        group_xyz_l2 = tf.sqrt(tf.reduce_sum(tf.square(group_xyz_l1), axis=-1, keepdims=True))  # (b,n,K,1)
        # group_feature_l2 = tf.sqrt(tf.reduce_sum(tf.square(group_feature_l1), axis=-1, keepdims=True))  # (b,n,K,1)
        local_feature = tf.concat([xyz_k, grouped_xyz, group_xyz_l1, grouped_feature_l1, group_xyz_l2], axis=-1)
        local_weight1 = tf_util.conv2d(local_feature, 32, kernel_size=[1, 1],
                                       padding='VALID', data_format='NHWC', stride=[1, 1],
                                       bn=bn, is_training=is_training,
                                       scope=scope + '0', bn_decay=bn_decay, weight_decay=weight_decay)
        local_weight2 = tf_util.conv2d(local_weight1, 1, kernel_size=[1, 1],
                                       padding='VALID', data_format='NHWC', stride=[1, 1],
                                       bn=bn, is_training=is_training,
                                       scope=scope + '1', bn_decay=bn_decay,
                                       weight_decay=weight_decay)  # grouped_feature.get_shape()[-1]
        
        feature_density = tf.squeeze(tf.reduce_sum(local_weight2, axis=2, keepdims=True), [2])  # (b,n,c)
        
        feature_density_max = tf.reduce_max(feature_density, ,axis=2, keepdims=True)
        feature_density_max = tf.maximum(feature_density_max, 1e-10)
        feature_density = feature_density/feature_density_max
        feature_density = find_feature_density(xyz, feature_density, l_cell)
        """
        weight_space=query_and_interpolation(xyz, feature, new_xyz,feature_density, l,layers,relative_xyz=relative_xyz)#(b,n,15,c)
        
        #batch_size = weight_space.get_shape()[0]
        #npoint = weight_space.get_shape()[1]
        #c=weight_space.get_shape()[3]

        #grouped_feature=tf.reshape(weight_space,(batch_size,npoint*size,size,size,c))

        new_points=tf_util.conv2d(weight_space, mlp, kernel_size=[1,weight_space.get_shape()[2].value],
                    padding='VALID',data_format='NHWC', stride=[1,1],
                    bn=bn, is_training=is_training,
                    scope=scope, bn_decay=bn_decay, weight_decay = weight_decay)#(b,n,1,c)
...
def feature_decoding_layer(xyz1, points1,xyz2, points2, size,layers, stride, mlp, radius, sigma, is_training, bn_decay, weight_decay, scope, bn=True, use_xyz = True):
    ...
    with tf.variable_scope(scope) as sc:
        ...
        """
        grouped_xyz, grouped_feature, _ = pointconv_util.grouping(points2, K, xyz2, xyz2, use_xyz=False)
        xyz_k = tf.tile(tf.expand_dims(xyz2, axis=2), (1, 1, K, 1))  # (b,n,K,3)
        feature_k = tf.tile(tf.expand_dims(points2, axis=2), (1, 1, K, 1))  # (b,n,K,c)
        group_xyz_l1 = xyz_k - grouped_xyz  # (b,n,K,3)
        grouped_feature_l1 = feature_k - grouped_feature  # (b,n,K,c)
        group_xyz_l2 = tf.sqrt(tf.reduce_sum(tf.square(group_xyz_l1), axis=-1, keepdims=True))  # (b,n,K,1)
        # group_feature_l2 = tf.sqrt(tf.reduce_sum(tf.square(group_feature_l1), axis=-1, keepdims=True))  # (b,n,K,1)
        ...
        weight_space=query_and_interpolation(xyz2,points2,xyz1,feature_density,l,layers)#(b,ndataset1,27,c)
...
def pointnet_fp_module(xyz1, xyz2, points1, points2, mlp, is_training, bn_decay, weight_decay, radius, last_layer, scope, bn=True):
    ...
    with tf.variable_scope(scope) as sc:
        if last_layer:
            ...
            l = tf.tile(tf.reshape(radius,(1,)),[batch_size])
            N_points = 64
            idx, pts_cnt = tf_grouping.query_ball_point(l, N_points, xyz2, xyz2)
            xyz_pts = tf_grouping.group_point(xyz2, idx)#(b,m,nsample,3)
            feature_pts = tf_grouping.group_point(points2, idx)#(b,m,nsample,c)
            grouped_density = tf.concat([xyz_pts, feature_pts], axis=3)
            feature_density=tf_util.conv2d(grouped_density, 32, kernel_size=[1,1],
                                          padding='VALID',data_format='NHWC', stride=[1,1],
                                          bn=bn, is_training=is_training,
                                          scope='density_decoding_net1', bn_decay=bn_decay, weight_decay = weight_decay)#(b,m,128,32)
            feature_density = tf.reduce_max(feature_density, axis=2)#(b,m,32)
            feature_density = nonlinear_transform(feature_density, [16, 1], scope='density_decoding_net2', is_training=is_training,
                                              bn_decay=bn_decay, weight_decay=weight_decay)

            layers=1
            relative_xyz=False
            #query_and_interpolation(xyz, feature, new_xyz,feature_density, l,layers,relative_xyz=relative_xyz)
            weight_space=query_and_interpolation(xyz2, points2, xyz1,feature_density, l,layers,relative_xyz=relative_xyz)#(b,n,15,c)
            interpolated_points=tf_util.conv2d(weight_space, mlp[0], kernel_size=[1,weight_space.get_shape()[2].value],
                    padding='VALID',data_format='NHWC', stride=[1,1],
                    bn=bn, is_training=is_training,
                    scope=scope, bn_decay=bn_decay, weight_decay = weight_decay)
            interpolated_points=tf.squeeze(interpolated_points, [2]) #(b,n,c)
        ...
        if last_layer:
            ...
            new_points1 = tf_util.conv2d(new_points1, mlp[-1], [1,1],
                                     padding='VALID', stride=[1,1],
                                     bn=bn, is_training=is_training,
                                     scope='conv_%d'%(2), bn_decay=bn_decay)
